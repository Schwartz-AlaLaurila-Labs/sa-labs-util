function [spikeTimes, spikeAmplitudes, refractoryViolations] = detectSpikes2(dataMatrix, varargin)
%
...
for tt = 1 : nTraces
    ...
    % cluster spikes
    clusteringData = [peakAmplitudes', 2*rebound.Left', 2*rebound.Right'];
    startMatrix = [median(peakAmplitudes) 2*median(rebound.Left) 2*median(rebound.Right);...
        max(peakAmplitudes) 2*max(rebound.Left) 2*max(rebound.Right)];
